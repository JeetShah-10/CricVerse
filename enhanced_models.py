...
# Model classes will be defined dynamically during initialization
...
ChatMessage = None
BookingAnalytics = None
SystemLog = None
WebSocketConnection = None

def init_enhanced_models(app, app_db):
    """Initialize enhanced models with proper db instance"""
    global db, CustomerProfile, PaymentTransaction, QRCode, Notification
    global MatchUpdate, ChatConversation, ChatMessage, BookingAnalytics
    global SystemLog, WebSocketConnection
    
    try:
        # Set the db instance
        db = app_db
        
        # Define all model classes dynamically
        ...
        globals()['CustomerProfile'] = CustomerProfile
        globals()['PaymentTransaction'] = PaymentTransaction
        globals()['QRCode'] = QRCode
        globals()['Notification'] = Notification
        globals()['MatchUpdate'] = MatchUpdate
        globals()['ChatConversation'] = ChatConversation
        globals()['ChatMessage'] = ChatMessage
        globals()['BookingAnalytics'] = BookingAnalytics
        globals()['SystemLog'] = SystemLog
        globals()['WebSocketConnection'] = WebSocketConnection
        
        # Create enhanced tables
        success = create_enhanced_tables(app_db)
        if success:
            print("✅ Enhanced models initialized successfully")
        else:
            print("⚠️ Enhanced models initialization completed with warnings")
...
def add_enhanced_columns():
    ...
    try:
        ...
        print("📝 Executing enhanced column migrations...")
        from app import db
        ...
        db.session.commit()
        print("✅ Enhanced column migrations completed!")
        
        return True
        
    except Exception as e:
        print(f"❌ Failed to execute enhanced columns: {e}")
        db.session.rollback()
        return False
