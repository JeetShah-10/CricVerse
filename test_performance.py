...
def test_home_page_performance():
    ...
    print("🚀 Testing CricVerse Home Page Performance")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "big bash" in content:
                print("✅ BBL content found")
            if "cricverse" in content:
                print("✅ CricVerse branding found")
            if "error" in content and "database" in content:
                print("⚠️  Database fallback mode detected (expected)")
        else:
    ...
    for resource in static_resources:
        try:
            ...
            if response.status_code == 200:
                print(f"✅ {resource} - {load_time:.2f}s")
            else:
    ...
    print("\n3️⃣ Testing with multiple requests...")
    total_time = 0
